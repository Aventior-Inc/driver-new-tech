...
sudo sed -i -e "s/HOST_NAME/${HOST_NAME}/g" nginx/driver.conf && sudo sed -i -e "s,STATIC_ROOT,$STATIC_ROOT,g" nginx/driver.conf && sudo sed -i -e "s,DIST_ROOT,$DIST_ROOT,g" nginx/driver.conf && sudo sed -e "s/driver-new-tech/${DJANGO_HOST}/g" nginx/driver.conf && sudo sed -e "s/driver-celery/${CELERY_HOST}/g" nginx/driver.conf && sudo sed -e "s/windshaft/$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' windshaft)/g" nginx/driver.conf
